    ...
    reducers: {
        ...
        onDeleteEvent: (state) => {
            if (state.activePreset) {
                state.presets = state.presets.filter(event => event.id !== state.activePreset.id);
                state.activePreset = null;
            }
        },
        onLoadEvent: (state, { payload = [] }) => {
            ...
            payload.forEach(event => {
                const exists = state.presets.some(dbEvent => dbEvent.id === event.id);
                if (!exists) {
                    state.presets.push(event);
                }
            })
            state.activePreset = null;
        },
        onLogoutPresets: (state) => {
...
export const { onSetActiveEvent, onSetNullEvent, onAddNewEvent, onUpdateEvent, onDeleteEvent, onLoadEvent, onLogoutPresets } = presetsSlice.actions;
